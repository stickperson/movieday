...
def convert_to_military(time_str):
    if 'a' in time_str and '12' in time_str:
        ...
        hour = 0
        result = str(hour) + ':' + minutes[:len(minutes)-2]
    elif 'a' in time_str:
        result = time_str[:len(time_str)-2]
...
def get_nearby(request):
    ...
    theaters = soup.find_all('div', class_='showtimes-theater')
    results = []
    theater_id = 0
    for theater in theaters:
        t = {}
        # all theaters name
        theater_div = theater.find('div', class_='showtimes-theater-details')
        info_div = theater_div.find('h3')
        theater_name = info_div.find('a').contents[0].strip()
        print theater_name
        t['name'] = theater_name
        t['movies'] = []
        # all movies names and showtimes at theater
        # movie_divs = theater.find('ul', class_='showtimes-movie-container')
        t['id'] = theater_id
        theater_id += 1
        title_divs = theater.find_all('div', class_='showtimes-movie-container')
        movie_id = 0
        for title_div in title_divs:
            m = {}
            m['id'] = movie_id
            movie_id += 1
            movie_name = title_div.find('h3').find('a').contents[0].strip()
            print movie_name
            # quick fix for getting duration of "NEW!" movies
            runtime_div = title_div.find('div', class_='showtimes-movie-rating-runtime')
            if runtime_div.contents[4].strip() == '':
                duration = '0 hr 0 min'
            else:
                duration = runtime_div.contents[4].strip().encode('ascii', 'ignore')
            duration_ls = duration.split()
            dur_hours = int(duration_ls[0])
            dur_minutes = int(duration_ls[2])
            ...
            m['showtimes'] = []
            m['display_showtimes'] = []
            showtime_links = title_div.find('div', class_='showtimes-times').find_all('a')
            for showtime_link in showtime_links:
                display_time = showtime_link.find('time', class_='timeInfo').contents[0]
